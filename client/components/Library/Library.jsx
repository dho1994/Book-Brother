      ...
      .then(response => {
        ...
        const orderedData = data.map((book, index) => {
          ...
          currBook.ready.then(() => {
            currBook.coverUrl()
              .then((results) => {
                if (Constants.EDITED_COVERS[book.URL]) {
                  document.getElementById(book.URL).src = `/${Constants.EDITED_COVERS[book.URL]}`;
                  book.coverURL = `/${Constants.EDITED_COVERS[book.URL]}`;
                } else if (results) {
          ...
          : displayBooks.filter(book => book.remainingText !== '').map(book => (
            <Card sx={{ width: '15rem', margin: '1rem', height: '25rem' }}>
              ...
              <CardContent sx={{ height: '4rem' }}>
                <Typography gutterBottom variant='subtitle1' component='div' sx={{ textAlign: 'center', verticalAlign: 'middle', padding: 'auto', fontSize: '1.8vh', height: '100%' }}>
                  {book.title}
                </Typography>
              </CardContent>
